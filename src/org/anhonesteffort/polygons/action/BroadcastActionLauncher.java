...
import org.anhonesteffort.polygons.AdminReceiver;
...
public class BroadcastActionLauncher extends BroadcastReceiver {
  private static final String TAG = "org.anhonesteffort.polygons.action.BroadcastActionLauncher";
  ...
  private void showNotification(Context context, boolean enter, String title) {
    ...
    // Start the PolygonMapActivity if the user clicks on the notification.
    Intent resultIntent = new Intent(context, PolygonMapActivity.class);
    TaskStackBuilder stackBuilder = TaskStackBuilder.from(context);
    stackBuilder.addParentStack(PolygonMapActivity.class);
  ...
  private void launchAction(Context context, Bundle polygonData, ActionBroadcastRecord action, String enter_exit) {
  ...
  public void onReceive(Context context, Intent intent) {
    ...
    if(intent.getAction().equals(ZoneService.POLYGON_ENTER) || intent.getAction().equals(ZoneService.POLYGON_EXIT)) {
      List<ActionBroadcastRecord> actions = applicationStorage.actions.getPolygonActions(intent.getExtras().getInt(ZoneService.POLYGON_ID));
    
      // Launch polygon enter & exit actions.
